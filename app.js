...
function toggleMobileMenu(btn){const nav=btn.parentElement.querySelector('.links');if(nav){nav.classList.toggle('open');btn.classList.toggle('open')}}
...
if(!window.__ivMenuBound){
  document.addEventListener('click',(e)=>{if(!e.target.closest('.nav-menu-wrap')) closeMobileMenu();});
  window.__ivMenuBound=true;
}
function header(active=''){return `<header class="nav"><div class="container nav-row"><a class="brand brand-wrap" href="index.html"><img src="assets/logo.jpg" alt="Iyarkai Vazhalai logo" class="brand-logo">Iyarkai Vazhalai</a><div class="nav-menu-wrap"><button class="menu-btn" aria-label="Open menu" onclick="toggleMobileMenu(this)">☰</button><nav class="links"><a href="index.html" onclick="closeMobileMenu()">Home</a><a href="shop.html" onclick="closeMobileMenu()">Shop</a><a href="about.html" onclick="closeMobileMenu()">About</a><a href="contact.html" onclick="closeMobileMenu()">Contact</a><a href="cart.html" onclick="closeMobileMenu()">Cart</a><span class="badge" data-cart-count>0</span></nav></div></div></header>`}
...
function productCard(p){
  ...
  return `<article class="card product-card">${photoHtml}<div class="product-info"><h3>${p.name}</h3><p class="small">${p.weight}</p><p class="product-price"><b>₹${p.price}</b></p><div class="product-actions"><button class="btn btn-primary" onclick="addToCart('${p.id}')">Add to cart</button> <a class="btn btn-outline" href="product.html?id=${p.id}">View</a></div></div></article>`;
}
const HERO_SLIDES=[
